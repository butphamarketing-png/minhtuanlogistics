...
module.exports = {
  ...
  saveNews: (data) => {
    writeJson("news-posts.json", data);
    // keep small mirror for legacy readers
    writeJson("news.json", Array.isArray(data) ? data.slice(0, 50) : data);
  },
  getSubpages: () => {
    const defaults = defaultSubpages();
    const saved = readJson("subpages.json", defaults);
    return {
      about: Array.isArray(saved.about) ? saved.about : [],
      services: Array.isArray(saved.services) ? saved.services : [],
    };
  },
  saveSubpages: (data) => writeJson("subpages.json", data),
  getSeoPages: () => {
    const defaults = defaultSeoPages();
    const saved = readJson("seo-pages.json", defaults);
    return { ...defaults, ...saved };
  },
  saveSeoPages: (data) => writeJson("seo-pages.json", data),
  ...
  getMedia: () => {
    ...
    const normalized = list.map((m, i) => {
      ...
      return {
        ...m,
        id: m?.name || `media-${Date.now()}-${i}`,
      };
    });
    if (changed) writeJson("media.json", normalized);
    return normalized;
  },
  addMedia: (entry) => {
    const list = readJson("media.json", []);
  ...
  deleteMedia: (id) => {
    ...
    if (!target) return { ok: false, list };
    const next = list.filter((m) => m !== target);
    writeJson("media.json", next);
    if (target.storage !== "r2" && target.name) {
      const fp = path.join(uploadsDir, target.name);
      try {
    ...
    return { ok: true, list: next, deleted: target };
  },
};
