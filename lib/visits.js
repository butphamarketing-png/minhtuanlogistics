...
const getMonthRange = (base = new Date()) => {
  ...
  const monthLabel = start.toLocaleDateString("vi-VN", { month: "long", year: "numeric" });
  return { year, month, daysInMonth, start, end, monthLabel };
};

const filterVisitsInRange = (visits, start, end) => {
...
/** Daily visit counts from day 1 through end of current month */
const buildMonthSeries = (visits, base = new Date()) => {
  const range = getMonthRange(base);
  const series = Array.from({ length: range.daysInMonth }, () => 0);
  const labels = Array.from({ length: range.daysInMonth }, (_, i) => String(i + 1));
  const monthVisits = filterVisitsInRange(visits, range.start, range.end);

  for (const v of monthVisits) {
    const d = new Date(v.createdAt);
    if (Number.isNaN(d.getTime())) continue;
    if (d.getFullYear() !== range.year || d.getMonth() !== range.month) continue;
    const day = d.getDate();
    if (day >= 1 && day <= range.daysInMonth) series[day - 1] += 1;
  }
...
const summarizeMonth = (visits, submissions = [], base = new Date()) => {
  const range = getMonthRange(base);
  const monthVisits = filterVisitsInRange(visits, range.start, range.end);
  const traffic = aggregateVisitors(monthVisits, submissions);
  const chart = buildMonthSeries(monthVisits, base);
  return {
    ...traffic,
    ...chart,
    monthLabel: range.monthLabel,
